...
def run_one_complex_level_hotpotqa(args):
    hotpot = json.load(open(args.task_path))
    agent_save_file = args.save_path
    ...
    llm = get_llm_backend(args.llm_name).run
    
    agent_cls = get_agent(args.agent_name)
    agents = [agent_cls(ques, ans, llm, args.max_context_len) for ques, ans in task_instructions]
    for agent in agents:
...
def run_one_complex_level_scienceqa():
    scienceqa = json.load(open(args.task_path))
    agent_save_file = args.save_path
    ...
    if os.path.exists(agent_save_file):
        ...
        utils.delete_error(agent_save_file) #agent_save_file에 non_error_sessions만 제공
    llm = get_llm_backend(args.llm_name, args.openai_key, args.use_openai).run
    agent_cls = get_agent(args.agent_name)
    agents = [agent_cls(ques, ans, llm, choices, cap, ocr, args.max_context_len) for ques, choices, ans, cap, ocr in task_instructions]
    for agent in agents:
        process_agent_run_step(agent)
        utils.log_agent(agent, agent_save_file)
    print(f'Finished Trial. Total: {len(agents)}')
...
def main(args):
    if args.task == "Hotpotqa":
        run_one_complex_level_hotpotqa(args)
    elif args.task == "Scienceqa":
        run_one_complex_level_scienceqa(args)
if __name__ == '__main__':
    ...
    assert args.agent_name in available_agent_names
    main(args)